...
def create_posts(post: schemas.Postcreate, db: Session = Depends(get_db),current_user:str=Depends(oauth2.get_current_user)):
    new_post = models.Post(owner_id=current_user.id,**post.dict())  # Creating new post from request data
    db.add(new_post)
    db.commit()
    db.refresh(new_post)  # Refresh to get the new post with its ID
    return  new_post
